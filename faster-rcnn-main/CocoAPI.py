...
class CocoData:
    ...
    def __init__(self, data: Union[None, dict] = None, file: str = ""):
        print(Fore.GREEN + f"Initiating {self.__class__.__name__} object...", 'cyan')
    ...
    def init(self, data: dict, file: str = ""):
        ...
        print('\t', Fore.GREEN + 'Images', 'yellow'), f' = {len(self.images)}'

        self.annotations = [CocoAnnotation(d) for d in data['annotations']]
        print('\t', Fore.GREEN + 'Annotations', 'yellow'), f' = {len(self.annotations)}'

        self.categories = [CocoCategory(d) for d in data['categories']]
        print('\t', Fore.GREEN + 'Categories', 'yellow'), f' = {len(self.categories)}'
